...
public class AdvancedSearchUseCase(ILLMRepository llmRepository, ILegifranceRepository legifranceRepository, IMapper mapper) : IAdvancedSearchUseCase
{
    public async Task<string?> Handle(PromptContentDto promptContentDto)
    {
        ...
        var fonds = promptContentDto.SelectedFonds.Select(x => mapper.Map<FondApiName>(x));
        
        var legifranceSearchResult = await legifranceRepository.SpecificSearchLawAsync(keysWord, fonds);
